...
class App(tk.Frame):
    ...
    def __init__(self, master=None, root_path=None, os_type='Windows', icon_file=None, ld_theme=None):
        ...
        header_frame.grid_rowconfigure(0, weight=1)  # Ensuring the header frame's row can expand.
        ...
        self.img_label = tk.Label(header_frame, image=self.img_tk)
    ...
    def setup_duration_boxes(self):
        ...
        self.time_input_frame = tk.Frame(self)
        self.time_input_frame.grid(row=3 + self.row_offset, column=0, padx=5)

        # Entry for Hours
        self.hour_entry = tk.Entry(self.time_input_frame, width=5, validate="key", vcmd=(self.master.register(validate_time_in), '%P'))
        self.hour_entry.insert(0, str(hours))  # Insert calculated hours
        self.hour_entry.grid(row=0 + self.row_offset, column=0, padx=2)

        # Entry for Minutes
        self.minute_entry = tk.Entry(self.time_input_frame, width=5, validate="key", vcmd=(self.master.register(validate_time_in), '%P'))
        ...
        # Entry for Seconds
        self.second_entry = tk.Entry(self.time_input_frame, width=5, validate="key", vcmd=(self.master.register(validate_time_in), '%P'))
    ...
    def create_widgets(self):
        ...
        self.sci_status_txt.grid(row=0 + self.row_offset, column=0, columnspan=1, pady=10)

        self.sci_status_label = tk.Label(self, text="Disconnected", font=('Roboto', 14), foreground=colors[self.ld_theme]['status'][self.connection_status])
        ...
        self.checkbox_frame.grid(row=5 + self.row_offset, column=0, rowspan=1)  # You can adjust the row and column based on your layout
        ...
        self.dac_value_entry = tk.Entry(self, width=5, validate="key", vcmd=(self.master.register(validate_dac_in), '%P'))
    ...
    def update_connection_status(self):
        ...
        if hasattr(self, 'ac_process') and self.ac_process:
            ...
            if self.connection_status == ConnectionStatus.DISCONNECTED:
                self.sci_status_label.config(text="Disconnected", fg=colors[self.ld_theme]['status'][self.connection_status])

            elif self.connection_status == ConnectionStatus.CONNECTING:
                self.sci_status_label.config(text="Connecting", fg=colors[self.ld_theme]['status'][self.connection_status])

            elif self.connection_status == ConnectionStatus.CONNECTED:
                self.sci_status_label.config(text="Connected", fg=colors[self.ld_theme]['status'][self.connection_status])

            elif self.connection_status == ConnectionStatus.ACQUIRING:
                self.sci_status_label.config(text="Acquiring", fg=colors[self.ld_theme]['status'][self.connection_status])
    ...
    def acquisition_btn_status_during_exp(self):
        self.start_button.config(state=tk.DISABLED)

        while not self.ac_process.acquisition_done:
            time.sleep(0.2)
